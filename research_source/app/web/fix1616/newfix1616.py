...
def regist_tweets_part(json_data):
    ...
    for name in json_data.keys():
        ...
        if(bm == day.month):
            if(bd == day.day):
                indatas["month"]["m"+sm]["day"]["d"+sd]["tweets"].append(
                {
                    "text":json_data[name]["text"],
                    "lon":json_data[name]["lon"],
                    "lat":json_data[name]["lat"],
                ...
                indatas["month"]["m"+sm]["day"]["d"+sd]={"tweets":[]}
                indatas["month"]["m"+sm]["day"]["d"+sd]["tweets"].append(
                {
                    "text":json_data[name]["text"],
                    "lon":json_data[name]["lon"],
...
def four_devide_tree(x0,x1,y0,y1,json_data,count):
    ...
    if(count == 4):
        ...
        d = regist_tweets_part(json_data)
        #pprint.pprint(d)
        location.append({
    #    'name':name,
    #    'lon':json_data[name]["lon"],
...
def main():
    ...
    four_devide_tree(sx0,sx1,sy0,sy1,json_data,count)
    ...
    for i in range(N):
        for t in range(N):
            ...
            #範囲内のデータdをindatasに加える=regist_tweets_part
            indatas = regist_tweets_part(indatas, d)
            location.append({
            'x0':x0,
            'x1':x0+lont,
            'y0':y0,
